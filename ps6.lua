...
local toggleLockRemote = inventoryFolder and inventoryFolder:FindFirstChild("ToggleLock")
if not toggleLockRemote then
    warn("ToggleLock Remote ไม่พบ, จะทำการขายโดยไม่ล็อกไอเทม")
end

-- ชื่อไอเทมที่อยากล็อก (แก้ได้ตามต้องการ)
local itemNamesToLock = {
    "Cinnabar",
    "Fire Opal",
    "Flarebloom",
    "Volcanic Core"
...
local targetSet = {}
for _, name in ipairs(itemNamesToLock) do
    targetSet[name:lower()] = true
end

-- Backpack reference (เพื่อสแกนไอเทม)
local backpack = player:WaitForChild("Backpack", 5)
if not backpack then
    warn("Backpack ไม่พบสำหรับผู้เล่น:", player.Name)
end

local function isItemLocked(item)
    local lockedAttr = item:GetAttribute("Locked")
    if type(lockedAttr) == "boolean" then
        return lockedAttr
    elseif type(lockedAttr) == "string" then
        return lockedAttr:lower() == "true"
    end
    return false
end

local function lockItem(item)
    if not toggleLockRemote then return end
    if not item or not item.Name then return end
    if not targetSet[item.Name:lower()] then return end
    ...
    if not success then
        warn("ล็อกไอเทมล้มเหลวสำหรับ:", item.Name, "เหตุผล:", err)
    end
end

-- สแกนของปัจจุบันใน Backpack แล้วล็อก
if backpack then
    for _, item in ipairs(backpack:GetChildren()) do
        lockItem(item)
    end
    -- ฟังการเพิ่มไอเทมใหม่แล้วล็อกอัตโนมัติ
    backpack.ChildAdded:Connect(function(child)
        task.defer(function()
            task.wait(0.1) -- รอให้ attribute ถูกเซ็ตถ้าจำเป็น
            lockItem(child)
        end)
    end)
end

local function lockTargetItems()
    if not backpack then return end
    for _, item in ipairs(backpack:GetChildren()) do
        lockItem(item)
    end
    -- หน่วงสั้นๆ ให้เซิร์ฟเวอร์ประมวลผลการล็อกก่อนขาย
    task.wait(0.2)
end
-- === จบส่วนล็อกไอเทม ===
...
local function toggleAutoSell()
    ...
    autosellThread = task.spawn(function()
        while autosellEnabled do
            pcall(function()
                -- ล็อกไอเทมเป้าหมายก่อนขายทั้งหมด
                lockTargetItems()
